...
fn resolves_private_method() {
    ...
    assert_eq!(
        ...
        [
            (
                7..225,
                Some("new ApiConnector".into()),
                Some("new ApiConnector".into())
            ),
            (
                56..142,
                Some("ApiConnector.#t".into()),
                Some("ApiConnector.#makeRequest".into())
            ),
            (
                142..165,
                Some("ApiConnector.#r".into()),
                Some("ApiConnector.#buildUrl".into())
            ),
            (
                165..224,
                Some("ApiConnector.get".into()),
                Some("ApiConnector.get".into())
            ),
        ]
    );
