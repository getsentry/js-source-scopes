...
fn hermes_scope_lookup() {
    ...
    let map =
        std::fs::read_to_string("tests/fixtures/hermes-metro/react-native-hermes.map").unwrap();

    let writer = SmCacheWriter::new("", &map).unwrap();
...
fn metro_scope_lookup() {
    ...
    let minified =
        std::fs::read_to_string("tests/fixtures/hermes-metro/react-native-metro.js").unwrap();
    let map =
        std::fs::read_to_string("tests/fixtures/hermes-metro/react-native-metro.js.map").unwrap();
